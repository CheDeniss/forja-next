...
const GameShowcase = ({ games }) => {
    ...
    useEffect(() => {
        const preloadFirstGame = async () => {
            ...
            const firstGame = await preloadGame(games[0]);
            setPreparedGames([firstGame]);
            setLoading(false);

            Promise.all(games.slice(1).map(preloadGame)).then(loadedGames => {
                setPreparedGames(prev => [...prev, ...loadedGames]);
            });
        };
    ...
    return (
        <div className={styles.container}>
            ...
            >
                ...
                >
                    <div className={`${styles.imgContainer} ${isImageLoaded ? styles.loaded : ''}`}>
                        <MinioImage
